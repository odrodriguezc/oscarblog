...
class  DataValidation
{
    ...
    const PHONE_PATTERN =      '/^(?:(?:\+|00)33[\s.-]{0,3}(?:\(0\)[\s.-]{0,3})?|0)[1-9](?:(?:[\s.-]?\d{2}){4}|\d{2}(?:[\s.-]?\d{3}){2})$/';
    private $errors;
    ...
    public  function phone(string $phone)
    {
        if (preg_match(self::PHONE_PATTERN, $phone) === 1) {
            ...
            $this->addError('Le numero de telephonoe n\'est pas valide. Il doit etre au format +33 xx xx xx xx xx');
            return false;
        }
    }
